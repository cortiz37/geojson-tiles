...
// converts GeoJSON feature into an intermediate projected JSON vector format with simplification data
    ...
    function convertLine(ring, out, tolerance, isPolygon, simple, tileSize) {
        ...
        for (var j = 0; j < ring.length; j++) {
            var x = projectX(ring[j][0], simple, tileSize);
            var y = projectY(ring[j][1], simple, tileSize);
...
// mercator-projected space into (extent x extent) tile space.
    ...
    function createTile(features, z, tx, ty, options) {
        ...
        for (var i = 0, list = features; i < list.length; i += 1) {
            ...
            var minX = feature.minX;
            var minY = feature.minY;
            var maxX = feature.maxX;
